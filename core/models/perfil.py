...
class PerfilManager(models.Manager):
    ...
    def vendedores_ativos(self):
        ...
        return self.filter(tipo=self.model.TipoUsuario.VENDEDOR, ativo=True, usuario__is_active=True)
    
    def ongs_ativas(self):
        """Retorna apenas perfis de ONGs ativas"""
        return self.filter(tipo=self.model.TipoUsuario.ONG, ativo=True, usuario__is_active=True)
    
    def clientes_ativos(self):
        """Retorna apenas perfis de Clientes ativos"""
        return self.filter(tipo=self.model.TipoUsuario.CLIENTE, ativo=True, usuario__is_active=True)
...
class Perfil(TimeStampedModel):
    ...
    def __str__(self):
        if self.tipo in [self.TipoUsuario.VENDEDOR, self.TipoUsuario.ONG] and self.nome_negocio:
