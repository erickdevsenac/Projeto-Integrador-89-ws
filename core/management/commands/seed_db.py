...
try:
    from core.models import (
        Perfil, CategoriaProduto, Produto, Pedido, PedidoVendedor,
    ...
    print(f"ERRO: Não foi possível importar os modelos. Verifique o arquivo 'core/models/__init__.py'. Detalhe: {e}")
    exit()
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Iniciando a semeadura do banco de dados...'))
        fake = Faker('pt_BR')
        ...
        models_to_delete = [
            ...
            Pedido, Cupom, Produto, CategoriaProduto
        ]
        ...
        profiles_to_create = []
        for user in User.objects.filter(is_superuser=False, perfil__isnull=True):
            profiles_to_create.append(Perfil(
                usuario=user, tipo=Perfil.TipoUsuario.CLIENTE,
                nome_completo=f"{user.first_name} {user.last_name}",
                telefone=fake.numerify('(##) #####-####'), endereco=fake.address()
            ))
        Perfil.objects.bulk_create(profiles_to_create)

        for i in range(NUM_VENDEDORES):
            user = User.objects.create_user(f'vendedor{i}', f'vendedor{i}@marketplace.com', 'password123')
            Perfil.objects.create(
                usuario=user, tipo=Perfil.TipoUsuario.VENDEDOR, nome_negocio=fake.company(),
                telefone=fake.numerify('(##) #####-####'), endereco=fake.address(), cnpj=fake.cnpj()
            )

        for i in range(NUM_ONGS):
            user = User.objects.create_user(f'ong{i}', f'ong{i}@parceira.org', 'password123')
            Perfil.objects.create(
                usuario=user, tipo=Perfil.TipoUsuario.ONG, nome_negocio=f'{fake.company()} (ONG)',
                telefone=fake.numerify('(##) #####-####'), endereco=fake.address(), cnpj=fake.cnpj(),
        ...
        categorias_prod = [CategoriaProduto(nome=nome, slug=slugify(nome)) for nome in CATEGORIAS_NOMES]
        ...
        for _ in range(NUM_PRODUTOS):
            preco = fake.pydecimal(left_digits=3, right_digits=2, positive=True, min_value=10, max_value=200)
            preco_original = None
            ...
            produtos_to_create.append(Produto(
                vendedor=random.choice(vendedores), categoria=random.choice(categorias_prod),
                nome=f"{fake.word().capitalize()} {fake.word()}", descricao=fake.paragraph(nb_sentences=3),
                preco=preco, preco_original=preco_original, motivo_desconto=motivo_desconto,
                codigo_produto=f'SKU-{uuid.uuid4().hex[:8].upper()}',
                data_validade=fake.future_date(end_date='+2y') if random.random() < 0.3 else None,
                quantidade_estoque=random.randint(0, 50), ativo=random.choices([True, False], weights=[0.9, 0.1])[0]
            ))
        Produto.objects.bulk_create(produtos_to_create)
        produtos = list(Produto.objects.filter(ativo=True, quantidade_estoque__gt=0))
        self.stdout.write(self.style.SUCCESS(f'Criadas {len(categorias_prod)} Categorias e {len(produtos_to_create)} Produtos.'))
        ...
        self.stdout.write('\nCriando Pedidos...')
        if not produtos:
            self.stdout.write(self.style.WARNING('Nenhum produto disponível para criar pedidos. Pulando etapa.'))
        else:
            for _ in range(NUM_PEDIDOS):
                cliente = random.choice(clientes)
                pedido_principal = Pedido.objects.create(
                    cliente=cliente, valor_total=Decimal('0.00'),
                ...
                vendedores_do_pedido = {p.vendedor for p in random.sample(produtos, k=random.randint(1, min(4, len(produtos))))}
                total_pedido = Decimal('0.00')

                for vendedor in vendedores_do_pedido:
                    produtos_do_vendedor_no_pedido = [p for p in produtos if p.vendedor == vendedor and p in random.sample(produtos, k=random.randint(1,3))]
                    if not produtos_do_vendedor_no_pedido: continue
                    
                    subtotal_vendedor = sum(p.preco * random.randint(1, 2) for p in produtos_do_vendedor_no_pedido)
                    sub_pedido = PedidoVendedor.objects.create(
                        pedido_principal=pedido_principal, vendedor=vendedor,
                        valor_subtotal=subtotal_vendedor, status=random.choice(PedidoVendedor.StatusPedidoVendedor.choices)[0]
                    )
                    ...
                    for produto in produtos_do_vendedor_no_pedido:
                        ...
                        itens_to_create.append(ItemPedido(
                            sub_pedido=sub_pedido, produto=produto,
            ...
            self.stdout.write(self.style.SUCCESS(f'Criados {NUM_PEDIDOS} Pedidos completos.'))
            
        self.stdout.write(self.style.SUCCESS('\nSEMEADURA DO BANCO DE DADOS CONCLUÍDA!'))
        
# Para executar, use o comando: python manage.py seed_db
